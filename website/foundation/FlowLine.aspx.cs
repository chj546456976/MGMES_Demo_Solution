...
public partial class foundation_FlowLine : System.Web.UI.Page
{
    ...
    protected void BtSave_Click(object sender, EventArgs e)
    {
        ...
        bool IsExit = mg_FlowLineBLL.CheckPositionByName(1, 0, posiname);
        if (IsExit)
        {
            bool flag = mg_FlowLineBLL.AddPositionByName(posiname);
            if (flag)
            {
                BindData();
            }
    ...
    protected void BtDel_Click(object sender, ImageClickEventArgs e)
    {
        ...
        bool flag = mg_FlowLineBLL.DelPositionByName(id);
        if (flag)
        {
    ...
    protected void BtSave_Click1(object sender, ImageClickEventArgs e)
    {
        ...
        #endregion
        bool IsExit = mg_FlowLineBLL.CheckPositionByName(2, id, name);
        if (IsExit)
        {
            bool flag = mg_FlowLineBLL.UpdatePositionByName(id, name);
            if (flag)
            {
                GridView1.EditIndex = -1;
                BindData();
